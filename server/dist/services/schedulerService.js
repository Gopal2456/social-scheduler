    ...
    cron.schedule("* * * * *", async () => {
        ...
        try {
            ...
            for (const post of postsToPublish) {
                try {
                    ...
                    if (accounts.length === 0) {
                        console.warn(`No connected zernio accounts found for post ${post._id}`);
                        continue;
                    }
                    ...
                    console.log(`Publishing post ${post._id} to Zernio with payload: ${post.mediaUrl ? "media included" : "no media"}`);
                    ...
                    console.log(`Post ${post._id} published successfully with Zernio post ID: ${publishedPost._id || publishedPost.id}`);
            ...
            if (postsToPublish.length > 0) {
                console.log(`Processed ${postsToPublish.length} posts at ${now.toISOString()}.`);
            }
        }
        catch (error) {
